...
type SchoolCardProps = { school: RankedSchool; rank: number; onOpen: () => void };
...
export function SchoolCard({ school, rank, onOpen }: SchoolCardProps) {
  ...
  return <article className="card group flex flex-col p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_40px_rgba(28,52,42,.08)]"><div className="mb-5 flex items-start justify-between"><button type="button" onClick={onOpen} className="flex items-center gap-3 text-left"><div className="avatar !size-10" style={{ background: school.presentation.color, color: school.presentation.tone }}>{school.presentation.initials}</div><div><span className="text-[10px] font-bold text-[var(--muted)]">{school.recommendationScore === null ? "준비 중" : `추천 #${rank}`}</span><h3 className="mt-0.5 text-[15px] font-bold">{name}</h3><p className="mt-0.5 flex items-center gap-1 text-[11px] text-[var(--muted)]"><MapPin size={11} />{location}</p></div></button></div>
    <button type="button" onClick={onOpen} className="mb-4 flex items-end justify-between rounded-xl bg-[var(--paper)] p-3 text-left"><div><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[var(--muted)]">프로필 매칭</p><p className="display mt-0.5 text-2xl font-bold text-[var(--green)]">{school.matchScore === null ? "—" : `${school.matchScore}%`}</p><p className="mt-1 text-[9px] text-[var(--muted)]">종합 평가 {school.overallScore === null ? "—" : `${school.overallScore} / 5.0`}</p></div><div className="text-right"><p className="text-[10px] text-[var(--muted)]">예상 월 비용</p><p className="mt-1 text-sm font-bold">{cost === null ? "준비 중" : formatKRW(cost)}</p></div></button>
